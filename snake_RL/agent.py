...
class Agent:
    ...
    def get_move(self, board, score, turns_alive, turns_to_starve, direction, head_position, body_parts):
        ...
        boardW = len(board)
        boardH = len(board[0])
        food = [(x, y) for x in range(len(board)) for y in range(len(board[0])) if board[x][y] == GameObject.FOOD]
        walls = [(x, y) for x in range(len(board)) for y in range(len(board[0])) if board[x][y] == GameObject.WALL]

        if len(self.route) <= 1:
            new_body = body_parts
            if self.eaten > 1:
                new_body = (*new_body, new_body[-1])
            elif self.eaten == 1:
                new_body = (head_position)
            graph = AStarGraph(walls, boardW, boardH, direction, new_body)
            start = (head_position, direction, tuple(body_parts))
            start_time = time.time()
            result, cost, eaten = AStarSearch(start, food[0], graph)
            print("Time is", time.time() - start_time)
            self.times.append(time.time() - start_time)
            self.n = self.n + 1
            # if (self.n == 30):
            #     print("Average ", sum(self.times) / len(self.times))
            #
            # if eaten: self.eaten += 1
            self.route = result
        next = self.route[1] if len(self.route) > 1 else self.route[0]
        self.route = self.route[1:]
        for move in [Move.LEFT, Move.STRAIGHT, Move.RIGHT]:
            new_direction = direction.get_new_direction(move)
            d = new_direction.get_xy_manipulation()
            t = (head_position[0] + d[0], head_position[1] + d[1])
            if t == next[0]:
                return move
    ...
    def on_die(self, head_position, board, score, body_parts):
        ...
        print("Average ", sum(self.times) / len(self.times))
        print("Games: %i; Average: %d" % (len(self.scores), sum(self.scores) / len(self.scores)))
